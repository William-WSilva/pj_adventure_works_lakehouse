...
def transform_fact_internet_sales(
    ...
    product_cost_history_df: DataFrame
) -> DataFrame:
    '''
    ...
    product_cost_history_df = deduplicate(product_cost_history_df, "ProductID")
    ...
    fact_internet_sales = (
        sales_order_detail_df.alias("sod")
            ...
            .join(product_cost_history_df.alias("pch"), "ProductID", "left")
    )
    ...
    fact_internet_sales = fact_internet_sales.select(
        ...
        (F.col("sod.OrderQty") * F.col("pch.StandardCost")).alias("TotalProductCost"),
